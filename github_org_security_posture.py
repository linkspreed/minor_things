#!/usr/bin/env python3
"""
GitHub-Organisations-Sicherheitslage. Eigener Vorschlag (nicht im urspruenglichen
Foundation Package als Einzelkontrolle benannt), setzt aber direkt R-06
("Hardware-key MFA mandatory for all maintainers, branch protection ... on
default branches") und R-15 ("Role-based access ... quarterly recertification") um.

Prueft: organisationsweite 2FA-Pflicht, Mitglieder ohne 2FA, externe
Kollaborateure, Branch-Protection auf dem Default-Branch jedes aktiven Repos.

Fuer die Org-weiten Punkte (2FA-Pflicht, Mitgliederliste, externe
Kollaborateure) braucht SRC_GH_TOKEN zusaetzlich den Scope 'read:org'
(klassischer PAT) bzw. "Organization permissions: Members (read)"
(feingranularer PAT). Fehlt der Scope, wird das als INFO uebersprungen -
der Lauf schlaegt dadurch NICHT fehl.
...
SRC_GH_TOKEN = env('SRC_GH_TOKEN')  # bewusst nicht 'required=True', siehe main()
...
def check_org_level(owner: str) -> list:
    findings = []
    r = http_get(f'https://api.github.com/orgs/{owner}', headers=HEADERS, timeout=30,
                 description='Org-Details abrufen', logger=log)
...
def check_branch_protection(owner: str, repos: list) -> list:
    ...
    for repo in repos:
        ...
        checked += 1
        r = http_get(f'https://api.github.com/repos/{owner}/{name}/branches/{branch}/protection',
                     headers=HEADERS, timeout=30, description=f'Branch-Protection ({name})', logger=log)
...
def send_final_report(level, all_findings, repos_count, duration, fatal_error=None):
    header = [
        ...
        f'GitHub-Org-Sicherheitslage - {datetime.now(timezone.utc).strftime("%d.%m.%Y %H:%M UTC")}',
        f'Repos gesamt: {repos_count} | Auffaelligkeiten: {len(all_findings)}',
        f'Dauer: {duration}s',
        '',
    ]
    if fatal_error:
    ...
    if GOOGLE_CHAT_WEBHOOK:
        try:
            send_google_chat(GOOGLE_CHAT_WEBHOOK,
                              f'{SEVERITY_ICON.get(level, "✅")} GitHub-Org-Sicherheitslage: {level} ({len(all_findings)} Auffaelligkeit(en)). Vollstaendiger Report per E-Mail.')
        except Exception:
            pass
