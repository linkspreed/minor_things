...
SECRET_PATTERNS = [('AWS Access Key ID', re.compile('\\bAKIA[0-9A-Z]{16}\\b'), 'CRITICAL', 'AWS-Zugriffsschluessel im Frontend-Code gefunden - AWS-Keys gehoeren NIEMALS ins Frontend.'), ('AWS Secret Access Key (Kontext)', re.compile('aws_secret_access_key["\\\']?\\s*[:=]\\s*["\\\']?[A-Za-z0-9/+=]{40}["\\\']?', re.I), 'CRITICAL', 'Moeglicher AWS-Secret-Access-Key im Frontend-Code gefunden.'), ('Google API Key', re.compile('\\bAIza[0-9A-Za-z\\-_]{35}\\b'), 'MEDIUM', 'Google-API-Key gefunden - PRUEFEN, ob dieser Key in der Google Cloud Console auf bestimmte Domains/APIs eingeschraenkt ist (falls ja, ist eine Veroeffentlichung im Frontend ueblich und unkritisch; falls nein, sofort einschraenken).'), ('Stripe Secret Key (live)', re.compile('\\bsk_live_[0-9a-zA-Z]{20,}\\b'), 'CRITICAL', 'Stripe LIVE Secret Key im Frontend gefunden - erlaubt vollen Zugriff auf euer Stripe-Konto (Zahlungen, Kundendaten). SOFORT im Stripe-Dashboard widerrufen.'), ('Stripe Secret Key (test)', re.compile('\\bsk_test_[0-9a-zA-Z]{20,}\\b'), 'MEDIUM', 'Stripe TEST Secret Key im Frontend gefunden - im Testmodus geringeres Risiko, sollte aber trotzdem nicht oeffentlich sein.'), ('GitHub Personal Access Token', re.compile('\\bgh[pousr]_[0-9A-Za-z]{36,}\\b'), 'CRITICAL', 'GitHub-Zugriffstoken im Frontend gefunden - erlaubt ggf. Zugriff auf private Repos/Aktionen. SOFORT widerrufen.'), ('Slack Token', re.compile('\\bxox[baprs]-[0-9A-Za-z\\-]{10,}\\b'), 'HIGH', 'Slack-Zugriffstoken im Frontend gefunden.'), ('SendGrid API Key', re.compile('\\bSG\\.[0-9A-Za-z_\\-]{22}\\.[0-9A-Za-z_\\-]{43}\\b'), 'CRITICAL', 'SendGrid-API-Key im Frontend gefunden - erlaubt Mailversand in eurem Namen.'), ('Mailgun API Key', re.compile('\\bkey-[0-9a-zA-Z]{32}\\b'), 'HIGH', 'Moeglicher Mailgun-API-Key im Frontend gefunden.'), ('Twilio Auth Token (Kontext)', re.compile('twilio[_-]?auth[_-]?token["\\\']?\\s*[:=]\\s*["\\\']?[0-9a-f]{32}["\\\']?', re.I), 'CRITICAL', 'Twilio-Auth-Token im Frontend gefunden - erlaubt SMS/Anrufe in eurem Namen.'), ('Private-Key-Block', re.compile('-----BEGIN (RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----'), 'CRITICAL', 'Ein privater kryptografischer Schluessel ist direkt im Frontend-Code eingebettet - das darf NIEMALS oeffentlich sein.'), ('Firebase Server-Key (Kontext)', re.compile('firebase[_-]?(server|admin)[_-]?key["\\\']?\\s*[:=]\\s*["\\\']?[A-Za-z0-9_\\-:]{30,}["\\\']?', re.I), 'CRITICAL', 'Moeglicher Firebase-Server-/Admin-Key im Frontend gefunden.'), ('Hartcodiertes JWT', re.compile('\\beyJ[A-Za-z0-9_-]{10,}\\.eyJ[A-Za-z0-9_-]{10,}\\.[A-Za-z0-9_-]{10,}\\b'), 'HIGH', 'Ein fest einprogrammiertes JSON-Web-Token (Zugriffstoken) wurde im Frontend gefunden - falls dieses Token echte Rechte hat, kann es von jedem Besucher ausgelesen und wiederverwendet werden.'), ('Generische Secret-Zuweisung', re.compile('(?:secret|api[_-]?key|access[_-]?token|client[_-]?secret)["\\\']?\\s*[:=]\\s*["\\\'][A-Za-z0-9_\\-]{20,}["\\\']', re.I), 'MEDIUM', 'Eine Variable mit einem "secret/key/token"-aehnlichen Namen und einem langen, zufaellig aussehenden Wert wurde gefunden - bitte manuell pruefen, ob es sich um einen echten, sensiblen Schluessel handelt.')]
SAFE_PUBLIC_PREFIXES = ('pk_live_', 'pk_test_', 'NEXT_PUBLIC_', 'VITE_PUBLIC_', 'PUBLIC_')
SCRIPT_SRC_RE = re.compile('<script[^"\\\']+["\\\']', re.I)
INLINE_SCRIPT_RE = re.compile('<script(?![^>]*\\bsrc=)[^>]*>(.*?)</script>', re.I | re.S)
...
def fetch(url: str) -> str:
    r = requests.get(url, timeout=REQUEST_TIMEOUT, allow_redirects=True, headers={'User-Agent': 'isms-frontend-secret-scan/1.0'}, stream=True)
    r.raise_for_status()
...
def scan_host(host: str) -> tuple:
    ...
    for js_url in collect_js_urls(base_url, html):
        try:
...
def send_final_report(level, all_findings, hosts_scanned, hosts_unreachable, zones_count, duration, fatal_error=None):
    ...
    header = [f'SEVERITY_LEVEL: {level}', f"Frontend-Secret-Scan (Live-Webseiten) - {datetime.now(timezone.utc).strftime('%d.%m.%Y %H:%M UTC')}", f'Zonen: {zones_count} | Hosts erfolgreich gescannt: {hosts_scanned} | Nicht per HTTP(S) erreichbar: {hosts_unreachable} | Funde: {len(all_findings)}', f'Dauer: {duration}s', '']
    if fatal_error:
    ...
    if GOOGLE_CHAT_WEBHOOK:
        try:
            send_google_chat(GOOGLE_CHAT_WEBHOOK, f"{SEVERITY_ICON.get(level, '✅')} Frontend-Secret-Scan: {level} ({len(all_findings)} Fund(e) auf {hosts_scanned} Live-Seiten). Vollstaendiger Report per E-Mail.")
        except Exception:
            pass
    _shred(SUMMARY_FILE)
