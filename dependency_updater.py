...
ECOSYSTEM_MARKERS = {'requirements.txt': 'pip', 'package.json': 'npm', 'pom.xml': 'maven', 'go.mod': 'gomod', 'Gemfile': 'bundler'}

def file_exists(owner: str, repo: str, path: str) -> bool:
    url = f'https://api.github.com/repos/{owner}/{repo}/contents/{path}'
    r = http_get(url, headers=HEADERS, timeout=30, description=f'Datei pruefen ({repo}/{path})', logger=log)
...
def create_dependabot_config(owner: str, repo: str, ecosystems: list):
    ...
    body = {'message': 'Add automated dependabot.yml (via zentrale Dependency-Updater-Action)', 'content': encoded}
    r = http_put(url, headers=HEADERS, json_body=body, timeout=30, description=f'dependabot.yml anlegen ({repo})', logger=log)
