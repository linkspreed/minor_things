...
def check_gitlab_sizes() -> list:
    ...
    threshold_kb = SIZE_WARNING_THRESHOLD_MB * 1024
    while True:
        r = http_get(f'{GITLAB_URL}/api/v4/projects', headers=headers, params={'membership': True, 'per_page': 100, 'page': page, 'statistics': True}, timeout=30, description='GitLab-Projekte auflisten (Statistik)', logger=log)
        r.raise_for_status()
        batch = r.json()
        if not batch:
            break
...
def check_drive_quota():
    ...
    about = with_retry(lambda: service.about().get(fields='storageQuota').execute(), 'Google-Drive-Speicherquota abfragen', logger=log)
