...
def shallow_clone(repo_name: str, clone_url: str) -> Path:
    ...
    if dest.exists():
        shutil.rmtree(dest)
    subprocess.run(['git', 'clone', '--depth', '1', '--quiet', clone_url, str(dest)], check=True, capture_output=True, text=True, timeout=300)
    return dest

def index_with_ripgrep(repo_path: Path) -> list:
    result = subprocess.run(['rg', '--files'], cwd=repo_path, capture_output=True, text=True, timeout=120)
    entries = []
...
def upload_index_to_drive(local_path: Path):
    ...
    existing = with_retry(lambda: service.files().list(q=query, fields='files(id)', supportsAllDrives=True, includeItemsFromAllDrives=True).execute(), 'Alten Index suchen', logger=log).get('files', [])
    media = MediaFileUpload(str(local_path), mimetype='application/json', resumable=True)
    with_retry(lambda: service.files().create(body={'name': INDEX_FILE_NAME, 'parents': [GDRIVE_FOLDER_ID]}, media_body=media, fields='id', supportsAllDrives=True).execute(), 'Neuen Index hochladen', logger=log)
    for f in existing:
        try:
