#!/usr/bin/env python3
"""
Externes Attack-Surface-Scanning (Policy 2.3 / Annex C).

Die "bekannten, erlaubten" Hostnamen kommen automatisch live aus Cloudflare
(ueber alle Zonen). subfinder findet zusaetzlich passiv weitere Subdomains;
alles, was subfinder findet, aber NICHT als Cloudflare-DNS-Eintrag existiert,
wird als "unerwartetes/veraltetes Asset" gemeldet (z.B. ein alter, vergessener
externer DNS-Eintrag, oder ein Fund aus einem anderen DNS-Provider).
httpx probt jeden bekannten Host auf HTTP-Status, TLS-Version und fehlende
Security-Header.
...
def run_subfinder(domain: str) -> list:
    ...
    proc = subprocess.run(['subfinder', '-d', domain, '-silent', '-json'],
                           capture_output=True, text=True, timeout=SUBFINDER_TIMEOUT)
    hosts = []
...
def run_httpx(hosts: list) -> dict:
    ...
    proc = subprocess.run(['httpx', '-silent', '-json', '-status-code', '-tls-grab', '-rc'],
...
def send_final_report(level, findings, known_count, discovered_count, unexpected_count, probed_count, zones_count, errors, duration, fatal_error=None):
    header = [
        ...
        f'Externer Attack-Surface-Scan - {datetime.now(timezone.utc).strftime("%d.%m.%Y %H:%M UTC")}',
        f'Zonen: {zones_count} | Bekannte Hosts (Cloudflare): {known_count} | Neu entdeckt (subfinder): {discovered_count} | Unerwartet: {unexpected_count} | Probed: {probed_count}',
        f'Dauer: {duration}s',
        '',
    ]
    if fatal_error:
    ...
    if GOOGLE_CHAT_WEBHOOK:
        try:
            send_google_chat(GOOGLE_CHAT_WEBHOOK,
                              f'{SEVERITY_ICON.get(level, "✅")} Attack-Surface-Scan: {level} '
                              f'({len(findings)} Auffaelligkeit(en), {unexpected_count} unerwartete Hosts). Vollstaendiger Report per E-Mail.')
        except Exception:
            pass
...
def main():
    ...
    try:
        ...
        for host in unexpected:
            findings.append(('MEDIUM', host,
                              'Von subfinder neu entdeckt, aber NICHT als aktueller Cloudflare-DNS-Eintrag vorhanden - bitte pruefen (evtl. anderer DNS-Provider, veralteter Eintrag oder Schatten-IT).'))
    ...
    send_final_report(level, findings, len(known_hosts), len(discovered),
                       len(unexpected), len(probed),
                       zones_count, errors, duration, fatal_error)
