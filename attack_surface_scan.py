...
HTTPX_TIMEOUT = int(env('HTTPX_TIMEOUT_SECONDS', default='400'))
HTTPX_PER_HOST_TIMEOUT = env('HTTPX_PER_HOST_TIMEOUT_SECONDS', default='8')
...
def run_httpx(hosts: list) -> dict:
    ...
    proc = subprocess.run(
        ['httpx', '-silent', '-json', '-status-code', '-tls-grab',
         '-timeout', HTTPX_PER_HOST_TIMEOUT, '-retries', '1'],
        input='\n'.join(hosts), capture_output=True, text=True, timeout=HTTPX_TIMEOUT,
    )
    if not proc.stdout.strip() and proc.returncode not in (0,):
        raise RuntimeError(
            f'httpx lieferte keine Ausgabe (Exit-Code {proc.returncode}). '
            f'stderr: {(proc.stderr or "(leer)")[:300]}'
        )
    results = {}
    for line in proc.stdout.splitlines():
        line = line.strip()
...
def send_final_report(level, findings, known_count, discovered_count, unexpected_count, probed_count, zones_count, errors, duration, fatal_error=None):
    header = [
        ...
        f'Zonen: {zones_count} | Bekannte Hosts (Cloudflare): {known_count} | Neu entdeckt (subfinder): {discovered_count} | Unerwartet: {unexpected_count} | Probed (nur bekannte Hosts): {probed_count}',
        f'Dauer: {duration}s',
        '',
    ]
    if fatal_error:
...
def main():
    ...
    try:
        ...
        probe_targets = sorted(known_set - ignore_hosts)
        try:
            probed = run_httpx(probe_targets)
        except Exception as e:
            errors.append(f'httpx: {e}')

        for host in unexpected:
            findings.append(('MEDIUM', host,
                              'Von subfinder neu entdeckt, aber NICHT als aktueller Cloudflare-DNS-Eintrag vorhanden - bitte pruefen (evtl. anderer DNS-Provider, veralteter Eintrag, Certificate-Transparency-Altlast oder Schatten-IT).'))
