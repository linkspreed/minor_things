...
from common import env, Redactor, SummaryLogger, list_github_repos, sanitize_gitlab_path, send_google_chat, with_retry
...
def ls_remote_refs(url: str, description: str) -> dict:
    ...
    def _attempt():
        result = subprocess.run(['git', 'ls-remote', '--heads', '--tags', url], check=True, capture_output=True, text=True, timeout=120)
