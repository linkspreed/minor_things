...
def clone_bare(repo_name: str, source_clone_url_with_token: str) -> Path:
    ...
    run_git(['git', 'clone', '--mirror', source_clone_url_with_token, str(bare_path)], description=f'klonen {repo_name}')
    return bare_path
...
def run_gitleaks(bare_path: Path, repo_name: str):
    ...
    def _attempt():
        cmd = [GITLEAKS_BIN, 'detect', '--source', str(bare_path), '--report-format', 'json', '--report-path', str(report_path), '--redact', '--no-banner', '--exit-code', '0']
...
def summarize_findings(findings):
    ...
    for f in findings:
        ...
        items.append({'rule': rule, 'file': f.get('File', '?'), 'line': f.get('StartLine', '?'), 'commit': (f.get('Commit', '') or '')[:12], 'author': f.get('Author', '?'), 'date': f.get('Date', '?')})
...
def process_repo(repo: dict):
    ...
    if findings:
        ...
        summary_log(f'--- {name} ---')
        summary_log(f'  {len(findings)} Fund(e): ' + ', '.join((f'{r}={c}' for r, c in sorted(by_rule.items(), key=lambda x: -x[1]))))
        for it in items[:MAX_DETAILS_PER_REPO]:
            summary_log(f"    [{it['rule']}] {it['file']}:{it['line']} (Commit {it['commit']}, Autor {it['author']}, {it['date']})")
...
def main():
    ...
    header = f'FINDINGS_COUNT: {total_findings}\nSecret-Scan Zusammenfassung (Gitleaks, Secret-Werte niemals im Klartext enthalten)\nRepos gescannt: {_repo_total} | mit Fund(en): {len(repos_with_findings)} | Fehler: {len(error_repos)}\nFunde gesamt: {total_findings}\nDauer: {int(duration)} Sekunden\n'
    ...
    subject = f'Secret-Scan Ergebnis ({total_findings} Funde)'
