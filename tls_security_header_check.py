...
def check_headers_and_redirect(host: str) -> list:
    findings = []
    try:
        r = requests.get(f'https://{host}', timeout=15, allow_redirects=True, headers={'User-Agent': 'isms-tls-header-check/1.0'})
        headers = {k.lower(): v for k, v in r.headers.items()}
        hsts = headers.get('strict-transport-security')
        if not hsts:
            findings.append(('HIGH', 'Kein Strict-Transport-Security-Header (HSTS fehlt).'))
        else:
            m = re.search('max-age=(\\d+)', hsts)
        ...
        return findings
    try:
        r2 = requests.get(f'http://{host}', timeout=10, allow_redirects=False, headers={'User-Agent': 'isms-tls-header-check/1.0'})
...
def send_final_report(level, all_findings, hosts_count, zones_count, duration, fatal_error=None):
    header = [f'SEVERITY_LEVEL: {level}', f"TLS-/Security-Header-Check - {datetime.now(timezone.utc).strftime('%d.%m.%Y %H:%M UTC')}", f'Zonen: {zones_count} | Hosts geprueft: {hosts_count} | Auffaelligkeiten: {len(all_findings)}', f'Dauer: {duration}s', '']
    if fatal_error:
    ...
    if GOOGLE_CHAT_WEBHOOK:
        try:
            send_google_chat(GOOGLE_CHAT_WEBHOOK, f"{SEVERITY_ICON.get(level, '✅')} TLS-/Header-Check: {level} ({len(all_findings)} Auffaelligkeit(en)). Vollstaendiger Report per E-Mail.")
        except Exception:
            pass
    _shred(SUMMARY_FILE)
